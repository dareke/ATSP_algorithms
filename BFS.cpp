...
pair<vector<int>, int> tspBranchAndBoundWithMST(const vector<vector<int>>& matrix, double& executionTime) {
    int matrixSize = matrix.size();
    queue<Node> queue;
    ...
    while (!queue.empty()) {
        ...
        // Odrzucamy węzły, które nie mogą prowadzić do lepszego rozwiązania
        if (current.lowerBound >= bestCost) {
            continue;
        }

        // Jeśli ścieżka jest pełna, zaktualizuj najlepsze rozwiązanie
        if (current.path.size() == matrixSize) {
            int completeCost = current.cost + matrix[current.path.back()][0]; // Powrót do miasta początkowego
        ...
        for (int nextCity = 0; nextCity < matrixSize; ++nextCity) {
            if (!current.visited[nextCity]) {
                ...
                // Dolne ograniczenie obliczane jako koszt MST dla nieodwiedzonych wierzchołków
                child.lowerBound = child.cost + calculateMSTCost(matrix, child.visited, mstCache);

                // Dodaj do kolejki tylko węzły, które mogą poprawić rozwiązanie
...
int main() {
    ...
    for (int i = 0; i < config.repetitions; ++i) {
        ...
        pair <vector<int>, int> result = tspBranchAndBoundWithMST(matrix, executionTime);
